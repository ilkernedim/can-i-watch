...
export default async function TVPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let show;
  try {
    show = await getMediaDetails('tv', id);
  } catch (error) {
    notFound();
  }
  
  const creator = show.created_by?.[0];
  const watchProviders = show['watch/providers']?.results?.TR || show['watch/providers']?.results?.US;
  const availableStreams = watchProviders?.flatrate || [];

  const trailer = show.videos?.results?.find((vid: any) => vid.type === "Trailer" && vid.site === "YouTube") || show.videos?.results?.[0];
            ...
            <div className="flex flex-col md:flex-row gap-8 items-end md:items-end">
              ...
              <div className="flex-1 space-y-4 mb-4">
                <div className="flex flex-wrap gap-2 items-center text-sm text-[#9cacba]">
                  ...
                  <span>•</span>
                  <span>{show.episode_run_time?.[0] || 45}m</span>
                </div>
        ...
        <div className="container mx-auto px-4 md:px-10 py-8 md:py-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8 flex flex-col gap-10">
              <div className="md:hidden flex gap-4">
                <div className="w-24 aspect-[2/3] relative rounded-lg overflow-hidden shrink-0">
                    {show.poster_path && (
                      <Image src={`https://image.tmdb.org/t/p/w200${show.poster_path}`} alt={show.name} fill className="object-cover" />
                    )}
                </div>
                <div className="flex flex-col justify-center">
                  <span className="text-[#9cacba] text-sm mb-1">Created by</span>
                  <span className="text-white font-semibold">{creator?.name || 'Unknown'}</span>
                </div>
              </div>
              ...
              <section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {show.images?.backdrops?.slice(0, 3).map((img: any, i: number) => (
            ...
            <div className="lg:col-span-4 flex flex-col gap-8">
              ...
              <div className="bg-surface-dark rounded-2xl p-6 border border-[#283139]">
                 <h3 className="text-white text-lg font-bold mb-4">TV Show Info</h3>
                 <div className="space-y-4">
                    <div>
                        <span className="block text-[#9cacba] text-xs font-medium uppercase tracking-wider mb-1">Original Name</span>
                        <span className="text-white text-sm font-medium">{show.original_name}</span>
                    </div>
                    <div>
                        <span className="block text-[#9cacba] text-xs font-medium uppercase tracking-wider mb-1">Status</span>
                        <span className="text-white text-sm font-medium">{show.status}</span>
                    </div>
                    <div>
                        <span className="block text-[#9cacba] text-xs font-medium uppercase tracking-wider mb-1">Seasons</span>
                        <span className="text-white text-sm font-medium">{show.number_of_seasons} ({show.number_of_episodes} Episodes)</span>
                    </div>
                    <div>
                        <span className="block text-[#9cacba] text-xs font-medium uppercase tracking-wider mb-1">Network</span>
                        <span className="text-white text-sm font-medium">{show.networks?.[0]?.name}</span>
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-surface-dark rounded-2xl p-6 border border-[#283139]">
                 <div className="flex items-end gap-3">
                    <span className="text-5xl font-black text-white tracking-tighter">{show.vote_average.toFixed(1)}</span>
                    <div className="flex flex-col pb-1">
                        <div className="flex gap-0.5 text-yellow-500 mb-1">
                           {[...Array(5)].map((_, i) => (
                               <Star key={i} className={`w-4 h-4 ${i < Math.round(show.vote_average / 2) ? 'fill-yellow-500 text-yellow-500' : 'text-gray-600'}`} />
                           ))}
                        </div>
                        <span className="text-[#9cacba] text-xs">{show.vote_count} ratings</span>
                    </div>
                 </div>
              </div>
          ...
          <div className="mt-16 border-t border-[#283139] pt-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {show.similar?.results?.slice(0, 5).map((sim: any) => (
                    <Link href={`/tv/${sim.id}`} key={sim.id} className="group">
                        <div className="aspect-[2/3] rounded-xl bg-[#283139] overflow-hidden mb-3 relative border border-white/5">
                            {sim.poster_path && (
                              <Image src={`https://image.tmdb.org/t/p/w500${sim.poster_path}`} alt={sim.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                            )}
                        ...
                        <h4 className="text-white font-bold group-hover:text-primary transition-colors truncate">{sim.name}</h4>
                        <p className="text-[#9cacba] text-sm">{sim.first_air_date?.split('-')[0]}</p>
                    </Link>
                ))}
            </div>
          </div>
